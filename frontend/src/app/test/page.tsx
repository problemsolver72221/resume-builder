...
import { JobAnalysis, ModelChoice, PublicAppSettings, resumeApi } from '@/lib/api';
...
export default function TestPage() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        setSelectedModel((current) => pickDefaultModel(settings, current));
  ...
  const modelOptions = listModelOptions(modelSettings);
  const hasAnyProvider = modelOptions.length > 0;
